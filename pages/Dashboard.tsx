...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Check and update scheduled status based on current time
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto animate-fadeIn">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-3xl font-black tracking-tight text-white uppercase">System Command</h2>
          <p className="text-slate-400 font-medium">Global creative orchestration hub and autonomous agent status.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Network Reach', val: '142.8k', change: '+12.5%', color: 'text-blue-400' },
          { label: 'Conversion Delta', val: '4.2%', change: '+0.8%', color: 'text-emerald-400' },
          { label: 'Asset Throughput', val: '892', change: '+24', color: 'text-purple-400' },
          { label: 'Campaign ROI', val: '3.8x', change: '+0.4x', color: 'text-amber-400' },
        ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="glass p-8 rounded-[3rem] border-slate-800/50">
            <h3 className="text-lg font-black uppercase tracking-tight mb-8 text-white">Engagement Intelligence</h3>
            <div className="h-80 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data}>
                  ...
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #334155', borderRadius: '24px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}
          ...
          <div className="glass p-8 rounded-[3rem] border-slate-800/50">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-black uppercase tracking-tight text-white">Campaign Pipeline</h3>
              <button onClick={() => navigate('/campaigns')} className="text-[10px] font-black text-purple-400 hover:text-purple-300 uppercase tracking-widest">View All</button>
            </div>
            <div className="space-y-4">
              {campaigns.length === 0 ? (
                <div className="py-12 text-center text-slate-600 border border-dashed border-slate-800 rounded-3xl">
                  <p className="text-xs font-bold uppercase tracking-widest opacity-50">No campaigns in pipeline.</p>
                </div>
                ...
                <div key={campaign.id} className="p-4 bg-slate-900/50 border border-slate-800 rounded-2xl flex items-center justify-between group hover:border-slate-700 transition-all">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {campaign.status === 'scheduled' && campaign.scheduledAt && (
                          <span className="text-[10px] font-mono text-blue-400 font-bold">
                            T- {new Date(campaign.scheduledAt).toLocaleString()}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="text-right">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Performance Index</p>
                    <p className="text-xs font-bold text-white">{(campaign.performance.reach / 1000).toFixed(1)}k Reach</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-8">
           <div className="glass p-8 rounded-[3rem] border-slate-800/50 h-full">
              <h3 className="text-xs font-black uppercase tracking-[0.3em] text-slate-500 mb-6 border-b border-slate-800 pb-4">Real-time Ingestions</h3>
              <div className="space-y-4">
                 {recentAssets.length === 0 ? (
                    <p className="text-xs text-slate-600 italic py-10 text-center">No recent assets generated.</p>
                    ...
                    <div key={i} className="flex items-center gap-4 p-3 bg-slate-900/80 rounded-2xl border border-slate-800 hover:border-purple-500/50 transition-all group cursor-pointer">
                       <div className="w-14 h-14 rounded-xl overflow-hidden bg-black flex-shrink-0 border border-slate-800 shadow-inner relative">
                          <img src={asset.url} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                          {asset.type === 'video' && <div className="absolute inset-0 bg-black/20 flex items-center justify-center text-[10px]">🎬</div>}
                       </div>
                       <div className="min-w-0 flex-1">
                          <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest truncate">Asset_{asset.id.slice(0, 8)}</p>
                          <p className="text-[9px] text-slate-500 font-bold uppercase mt-1">{asset.type} • {new Date(asset.createdAt).toLocaleDateString()}</p>
                       </div>
                    </div>
                 ))}
                 <button 
                  onClick={() => navigate('/library')}
                  className="w-full py-4 bg-slate-800/50 text-[10px] font-black uppercase tracking-widest text-purple-400 hover:bg-slate-800 hover:text-purple-300 rounded-2xl border border-slate-800/50 transition-all"
                 >
                   Open Repository Vault →
                 </button>
              </div>

              <div className="mt-12 space-y-4">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-slate-500 mb-6 border-b border-slate-800 pb-4">Transmission Pulse</h3>
                <div className="space-y-3">
                  {recentPosts.length === 0 ? (
                      <p className="text-xs text-slate-600 italic py-10 text-center">No active social broadcasts.</p>
                  ) : recentPosts.map((post, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-slate-900/80 rounded-2xl border border-slate-800">
                        <div className="flex items-center gap-3">
                            <span className="text-lg opacity-80">{post.platform === 'twitter' ? '𝕏' : post.platform === 'linkedin' ? '💼' : '📱'}</span>
                        ...
                        <span className={`text-[9px] font-black px-3 py-1 rounded-full border ${
                            post.status === 'published' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-blue-500/10 text-blue-500 border-blue-500/20 animate-pulse'
                        }`}>
                  ...
                  <button 
                    ...
                    className="w-full py-4 bg-slate-800/50 text-[10px] font-black uppercase tracking-widest text-blue-400 hover:bg-slate-800 hover:text-blue-300 rounded-2xl border border-slate-800/50 transition-all"
                  >
